%RTBPlotGYM Plot utilities for Robotics Toolbox
        ...
        function th = install_teach_panel(name, robot, q, opt)
            ...
            if isempty(c)
                ...
                if ~isempty(c)
                    ...
                    error('RTB:RTBPlotGYM:install_teach_panel', 'no window found');
                end
            else
            ...
            set(teachhandles.fig, 'ResizeFcn', @(src,event) RTBPlotGYM.resize_callback(robot, teachhandles));
...
%                 end
            ...
            uicontrol(panel, 'Style', 'pushbutton', ...
                ...
                'FontUnits', 'normalized', ...
                'FontSize', 0.7, ...
                'CallBack', @(src,event) RTBPlotGYM.quit_callback(robot, teachhandles), ...
            ...
            if isfield(opt, 'record') && ~isempty(opt.record)
                uicontrol(panel, 'Style', 'pushbutton', ...
                    ...
                    'FontUnits', 'normalized', ...
                    'FontSize', 0.6, ...
                    'CallBack', @(src,event) RTBPlotGYM.record_callback(robot, teachhandles), ...
                    'BackgroundColor', 'red', ...
            ...
            for j=1:n
                ...
                set(teachhandles.edit(j), ...
                    'Interruptible', 'off', ...
                    'Callback', @(src,event)RTBPlotGYM.teach_callback(src, name, j, teachhandles));
                ...
                addlistener(teachhandles.slider(j), 'ContinuousValueChange', ...
                    @(src,event)RTBPlotGYM.teach_callback(src, name, j, teachhandles) );
                
            end
            
            % refresh the display
            RTBPlotGYM.teach_callback([], name, [], teachhandles);
        ...
        function cyl(ax, r, extent, color, offset, varargin)
            %RTBPlotGYM.cyl Draw a cylinder
            %
            ...
            %
            % See also surf, RTBPlotGYM.box.
            
            n = 20;
            theta = (0:n)/n*2*pi;
            
            RTBPlotGYM.draw_shape(ax, r, extent, color, offset, theta, varargin{:});
        end
        
        
        function box(ax, r, extent, color, offset, varargin)
            %RTBPlotGYM.box  Draw a box
            %
        ...
        function create_floor(opt)
            ...
            if ~isempty(opt.floorimage)
                RTBPlotGYM.create_image_floor(opt)
            else
                RTBPlotGYM.create_tiled_floor(opt)
            end
        end
        ...
        function opt = plot_options(robot, optin)
            ...
            if ~isempty(opt.reach)
                ...
                assert(~isempty(robot), 'RTB:RTBPlotGYM:plot_options', 'robot must be defined to estimate reach');
                L = robot.links;
                ...
                for j=1:robot.n
                    if L(j).isrevolute
                        ...
                        if ~isempty(L(j).qlim)
                            ...
                            assert(all(L(j).qlim >= 0),  'RTB:RTBPlotGYM:plot_options', 'prismatic joint %d qlim values cannot be negative', j)
                            ...
                            if isempty(opt.workspace)
                                ...
                                error('RTB:RTBPlotGYM:plot_options', 'prismatic joint %d has no qlim parameter set, set it or specify ''workspace'' plot option', j);
                            else
                                reach = NaN;
                            end
                        end
